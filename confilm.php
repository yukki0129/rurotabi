		...
		if($min_kyot <= $str[$i][5]){ //Kyoto
			if($min_date <= $str[$i][6]){ 
				if($min_hide <= $str[$i][7]){
					if($min_natu <= $str[$i][8]){
						if($min_ligh <= $str[$i][9]){
							print("<table align='center' border='0' cellspacing='0' bordercolor='#000000' width='80%'>");
							print("<tr><th>[".$str[$i][1]."]</th></tr>");
							print("<tr><td>".$str[$i][2]."</td></tr>");
							$needtime_h =  floor ($str[$i][4] / 60); 
							$needtime_m = $str[$i][4] % 60; 
							$needtime = $needtime_h."時間".$needtime_m."分";
							print("<tr><td>所要時間：".$needtime."</td></tr>");
							print('<tr><td><a href="first.html?twi" data-ajax="false" class="ui-btn" data-transition="slide">詳細を見る</a></td></tr>');
...
if($agreement_count == 0){
	print("<br/><div align='center'><strong>申し訳ありません！</strong><br/>条件にマッチするコースがありませんでした。<br/>コースは随時追加いたしますので、またよろしくお願い致します。<br/><img src='./img/chara/notfound2.png'></div>");
}else{
